...
function bulkApplyApplications (args, callback) {
  var seneca = this;
  var sendEmail = seneca.export('queues/sendEmail')['index'];
...
//            }
           ...
           sendEmail({cmd: 'enqueue', name: 'bulk-apply-applications-kue', msg: _.clone({role: 'cd-dojos', cmd: 'send_email', payload: payload}, function(err, res) {
             if (err) {
        ...
        function sendDojoEmail () {
          ...
          if (eventData.notifyOnApplicant) {
            ...
            sendEmail({cmd: 'enqueue', name: 'bulk-apply-applications-kue', msg: _.clone({role: 'cd-dojos', cmd: 'send_email', payload: dojoPayload}, function(err, res) {
              if (err) {
               done(err);
              }
            })});
        ...
        function sendParentsEmail () {
          ...
          sendEmail({cmd: 'enqueue', name: 'bulk-apply-applications-kue', msg: _.clone({role: 'cd-dojos', cmd: 'send_email', payload: parentPayload}, function(err, res) {
            if (err) {
             done(err);
            }
          })});
