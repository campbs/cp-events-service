    ...
    function retrieveTicketsData (profiles, event, session, dojo, done) {
      async.map(applications, function (application, cb) {
        ...
        switch (applications[0].status) {
          case 'pending':
            dojoCode = 'dojo-need-approval-';
            emailCode = 'ticket-application-received-';
            break;
          case 'approved':
            dojoCode = 'dojo-event-booked-';
            emailCode = 'ticket-application-approved-';
            break;
          case 'cancelled':
            dojoCode = 'dojo-event-cancelled-'
        ...
        return done(null, profiles, event, session, dojo, tickets, emailCode, dojoCode);
      });
    }

    function sendAllEmails(profiles, event, session, dojo, tickets, emailCode, dojoCode, done){
      var emailSubject;
      //event..
      ...
      var generalPayload = {
        ...
        async.parallel([function sendEmail(pcB){
              ...
              async.eachSeries(profiles, function (profile, cb) {
                ...
                var sendEmailPayload= _.merge(emailPayload,generalPayload);
                iterator++;
                if (!profile.email){
                  return emailParents(cb);
                }

                seneca.act({role: 'kue-queue', cmd: 'enqueue', name: 'all-bulk-apply-emails', msg: _.clone({role: 'cd-dojos', cmd: 'send_email', payload: sendEmailPayload}, function(err,res){
                    if (err) return cb(err);
                  ...
                  function emailParents (cb) {
                    ...
                    async.eachSeries(profile.parents, function (parent, cb) {
                        sendEmailPayload.to = parent.email;
                        if (!_.contains(parentsEmailed, sendEmailPayload.to) && sendEmailPayload.to !== profile.email) {
                          parentsEmailed.push(sendEmailPayload.to);
                          seneca.act({role: 'kue-queue', cmd: 'enqueue', name: 'all-bulk-apply-emails', msg: _.clone({role: 'cd-dojos', cmd: 'send_email', payload: sendEmailPayload})});
          ...
          function sendDojoEmail(pcb){
            if(eventData.emailNotifications){
                ...
                var dojoPayload = {
                  ...
                  code: dojoCode,
                  subject:emailSubject,
                  from:'The CoderDojoTeam <info@coderdojo.org>',
                  replyTo : 'The CoderDojoTeam <info@coderdojo.org>',
                  content:{
                    applicantName: dojo.name
                  }
                };
                var sendDojoPayload= _.merge(dojoPayload,generalPayload);

                seneca.act({role: 'kue-queue', cmd: 'enqueue', name: 'all-bulk-apply-emails', msg: _.clone({role: 'cd-dojos', cmd: 'send_email', payload: sendDojoPayload})}  , function (err, res) {
                  if (err) {
                    return err;
                  }
                  else
