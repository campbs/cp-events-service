      ...
      async.parallel([
        ...
        function sendApplicantsEmail () {
          ...
          async.each(profiles, function (profile, profileCb) {
            ...
            if (!profile.email) {
              ...
              //move to next applicant if there are no parents for this applicant either (We have no one to email)
              ...
              if (_.isEmpty(profile.parents)) {
                ...
                //we email the parent email if no email is found for this applicant

                payload.to = profile.parents[0].email || null;
            ...
            seneca.act({role: 'kue-queue', cmd: 'enqueue', name: 'bulk-apply-applications-kue', msg: _.clone({role: 'cd-dojos', cmd: 'send_email', payload: payload}, function(err, res) {

              if (err) {

                return profileCb(err);

              } else {

                return profileCb(res);
        ...
        function sendDojoEmail () {
          ...
          if (eventData.emailNotifications) {
            ...
            seneca.act({role: 'kue-queue', cmd: 'enqueue', name: 'bulk-apply-applications-kue', msg: _.clone({role: 'cd-dojos', cmd: 'send_email', payload: dojoPayload}, function (err, res) {
        ...
        function sendParentsEmail () {
          ...
          async.each(profiles, function(profile, profileCb) {
            ...
            //if this profile has parents, it is a child

            if (!_.isEmpty(profile.parents)) {

              childrensNames.push(profile.name);

              parentName = profile.parents[0].name;
          ...
          seneca.act({role: 'kue-queue', cmd: 'enqueue', name: 'bulk-apply-applications-kue', msg: _.clone({role: 'cd-dojos', cmd: 'send_email', payload: parentPayload}, function (err, res) {
