            ...
            process.nextTick(function () {
              ...
              async.eachSeries(profiles, function (profile, cb) {
                var emailPayload = {
                 ...
                 seneca.act({role: 'cd-dojos', cmd: 'send_email', payload: sendEmailPayload}, function (err, res) {
                   if (err) return cb(err);
                    ...
                    async.eachSeries(profile.parents, function (parent, cb) {
                      if (!_.isObject(parent)) {
                        seneca.act({role: 'cd-users', cmd: 'load', id: parent, user: requestingUser}, function (err, parentUser) {
                          if (err) return cb(err);
                          sendEmailPayload.to = parentUser.email;
                          if (!_.contains(parentsEmailed, sendEmailPayload.to) && sendEmailPayload.to !== profile.email) {
                            ...
                            seneca.act({role: 'cd-dojos', cmd: 'send_email', payload: sendEmailPayload}, cb);
                          } else {
                            return cb();
                          }
                        });
          ...
          function sendDojoEmail(pcb){
            process.nextTick(function () {
              if(eventData.emailNotifications){
                 ...
                 seneca.act({role: 'cd-dojos', cmd: 'send_email', payload: sendDojoPayload}, function (err, res) {
                   if (err) {
                     return err;
                   }
                   else
