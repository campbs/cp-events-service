...
function bulkApplyApplications (args, callback) {
  ...
  function generateEmailContent (applications, done) {
    ...
    function sendEmail (profiles, event, session, dojo, tickets, emailCode, dojoCode, done) {
...
//test
     ...
     function sendDojoEmail(profiles, event, session, dojo, tickets ,emailCode, dojoCodeIn, done) {
      process.nextTick(function () {

        if(eventData.emailNotifications){
            var dojoEmailSubject;
            var dojoCode = dojoCodeIn;
            dojoEmailSubject = dojoEmailSubjectData[applications[0].status];
            var eventDate;
            var firstDate = _.first(event.dates);
            var lastDate = _.last(event.dates);
            var startTime = moment.utc(firstDate.startTime).format('HH:mm');
        ...
        seneca.act({role: 'cd-dojos', cmd: 'send_email', payload: payload2}, function (err, res) {
          if (err) {
            return err;
          }
          else
